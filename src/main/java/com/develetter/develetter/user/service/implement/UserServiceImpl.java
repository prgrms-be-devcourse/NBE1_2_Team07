...
//            certificationRepository.delete(ㅇcertificationEntity);
    ...
    public ResponseEntity<? super DeleteIdResponseDto> deleteId(DeleteIdRequestDto dto) {
        try {
            String userId=dto.getId();
            UserEntity userEntity=userRepository.findByUserId(userId);
            if(userEntity==null) return DeleteIdResponseDto.idNotFound();
            ...
            e.printStackTrace();
            return DeleteIdResponseDto.databaseError();
        }
        return DeleteIdResponseDto.success();
    }
